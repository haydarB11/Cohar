...
class StartingQuizService {
    ...
    async add() {
        try {
            const quiz = await StartingQuiz.create(this);
            return {
                data: quiz,
    ...
    static async checkSuccess(data) {
        try {
            const quiz = await StartingQuiz.findOne({
                where: {
                    id: data.quiz_id,
                }
            });
    ...
    static async getForOneCourse(course_id) {
        try {
            const quiz = await StartingQuiz.findOne({
                include: [
                    {
    ...
    static async edit(data) {
        try {
            const quiz = await StartingQuiz.findByPk(data.id);
    ...
    static async delete(id) {
        try {
            const deletedQuiz = await StartingQuiz.destroy({
                where: {
                    id: id
                }
            });
...
module.exports = { StartingQuizService };